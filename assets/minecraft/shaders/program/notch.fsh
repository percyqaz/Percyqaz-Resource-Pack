...
void main() {
    ...
    vec4 noise = texture2D(DitherSampler, steppedCoord * halfSize / 2.0);
    vec4 col = texture2D(DiffuseSampler, steppedCoord) + noise * vec4(1.0/12.0, 1.0/12.0, 1.0/6.0, 1.0);
    float r = float(int(col.r*16.0))/16.0;
    float g = float(int(col.g*16.0))/16.0;
    float b = float(int(col.b*16.0))/16.0;
