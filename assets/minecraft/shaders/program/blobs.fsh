...
void main(){
    ...
    vec4 uu = texture2D(DiffuseSampler, texCoord + vec2(              0.0, -oneTexel.y * 0.0));
    vec4 dd = texture2D(DiffuseSampler, texCoord + vec2(              0.0,  oneTexel.y * 0.1));
    vec4 ll = texture2D(DiffuseSampler, texCoord + vec2(-oneTexel.x * 0.0,               0.0));
    vec4 rr = texture2D(DiffuseSampler, texCoord + vec2( oneTexel.x * 0.0,               0.0));
